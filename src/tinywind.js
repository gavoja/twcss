...
let classParser
function updateClassParser () {
  classParser = new RegExp(`((?<mq>${[...MEDIA_QUERIES.keys()].join('|')}):)?((?<state>${STATES.join('|')}):)?(?<util>[^\\[]+(\\[(?<value>[^\\]]+)\\])?)`)
}
...
function addRule(instance, cls) {
  ...
  const { mq, state, util, value } = cls.match(classParser).groups
  ...
  if (mq) {
    ...
    rule = `${MEDIA_QUERIES.get(mq)} { .${escapeClass(mq)}\\:${utilWithState} ${css} }`
  } else {
